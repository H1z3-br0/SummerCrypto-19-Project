...
class Lattice:
    ...
    @staticmethod
    def _gso_vectors(B):
        Ортогонализация Грама-Шмидта списка векторов B (список Sage-векторов).
        Вынесено в статический метод, чтобы им могли пользоваться и gso(),
        и lll() — во втором случае векторы меняются внутри цикла, и
        self.basis на это время недействителен как источник данных.
        """
        n = len(B)
        Bstar = [B[0]]
